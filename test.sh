...
softhsm2-util --module ${MODULE} --slot 0 --init-token --label silly_signer --pin 1234 --so-pin 1234 > slot-assignment.txt
...
softhsm2-util --module "${MODULE}" --slot 1 --init-token --label entropic_ecdsa --pin 1234 --so-pin 1234 > slot-assignment.txt
SLOT_ASSIGNMENT=$(sed 's/.*to slot \(.\+\)/\1/' slot-assignment.txt)
